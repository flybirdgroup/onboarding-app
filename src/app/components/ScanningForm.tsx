...
const ScanningForm = () => {
  const [data, setData] = useState<ScanningStatus[]>([]);
  const [filters, setFilters] = useState<Partial<ScanningStatus>>({});
  ...
  const handleFilterChange = (column: keyof ScanningStatus, value: string) => {
  ...
  const filteredData = useMemo(() => {
    return data.filter(item => {
      return Object.entries(filters).every(([key, value]) => {
        ...
        const itemValue = item[key as keyof ScanningStatus];
        return String(itemValue).toLowerCase().includes(String(value).toLowerCase());
      });
    });
  }, [data, filters]);

  const columns: (keyof ScanningStatus)[] = ['EIM', 'Github', 'PR', 'ScanCategory', 'Link', 'Score'];
